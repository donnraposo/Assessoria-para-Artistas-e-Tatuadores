...
def _build_proposal(**overrides) -> GuestProposal:
    artist_user = User.objects.create_user(
        email=overrides.pop("artist_email", "artist-p@example.com"), full_name="Artist"
    )
    studio_user = User.objects.create_user(
        email=overrides.pop("studio_email", "studio-p@example.com"), full_name="Studio"
    )
...
def test_draft_proposal_can_be_declined_with_reason() -> None:
    advisory = User.objects.create_user(email="admin-decline@example.com", full_name="Admin")
...
def test_proposal_transition_requires_reason() -> None:
    advisory = User.objects.create_user(email="admin-reason@example.com", full_name="Admin")
    proposal = _build_proposal()
...
def test_confirmed_proposal_cannot_be_declined() -> None:
    advisory = User.objects.create_user(email="admin-confirmed@example.com", full_name="Admin")
    proposal = _build_proposal()
    GuestProposalService.mark_ready(proposal)
...
def test_advisory_can_cancel_proposal_via_api() -> None:
    advisory_user = User.objects.create_user(email="admin-api@example.com", full_name="Admin")
...
def test_advisory_can_confirm_proposal_via_api() -> None:
    advisory_user = User.objects.create_user(email="admin-confirm@example.com", full_name="Admin")
    advisory_role = Role.objects.create(code=RoleType.ADVISORY, name="Advisory")
    UserRole.objects.create(user=advisory_user, role=advisory_role)
    proposal = _build_proposal()
    GuestProposalService.mark_ready(proposal)
...
def test_advisory_can_add_second_studio_to_confirmed_guest() -> None:
    advisory_user = User.objects.create_user(email="admin-studio@example.com", full_name="Admin")
    advisory_role = Role.objects.create(code=RoleType.ADVISORY, name="Advisory")
    UserRole.objects.create(user=advisory_user, role=advisory_role)
    second_studio_owner = User.objects.create_user(
        email="studio2-p@example.com", full_name="Second Studio Owner"
    )
...
def test_overlapping_studio_period_is_rejected() -> None:
    advisory_user = User.objects.create_user(email="admin-overlap@example.com", full_name="Admin")
    second_studio_owner = User.objects.create_user(
        email="studio3-p@example.com", full_name="Second Studio Owner"
    )
    second_studio = Studio.objects.create(
        owner=second_studio_owner,
        name="Second Room",
