...
class StudioBookingService:
    ...
    def confirm(request: StudioBookingRequest, advisory_user) -> StudioBooking:
        StudioBookingService._lock_studio_schedule(str(request.studio_id))
        ...
        overlapping = StudioBooking.objects.filter(
        ...
        if locked.workstation_id:
            conflict = overlapping.filter(request__workstation=locked.workstation_id).exists()
        else:
            capacity = max(
                Workstation.objects.filter(studio=locked.studio, is_active=True).count(), 1
            )
            conflict = overlapping.count() >= capacity
        if conflict:
        ...
        workstation_id = str(locked.workstation_id) if locked.workstation_id else None
        AuditEvent.objects.create(
            actor=advisory_user,
            action="studio_booking.confirmed",
            resource_type="StudioBooking",
            resource_id=str(booking.id),
            changes={"workstation_id": workstation_id},
        )
        return booking

    @staticmethod
    def _lock_studio_schedule(studio_id: str) -> None:
        with connection.cursor() as cursor:
            cursor.execute("SELECT pg_advisory_xact_lock(hashtext(%s))", [studio_id])
