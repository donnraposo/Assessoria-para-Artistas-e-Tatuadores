...
class AccommodationCollectionView(APIView):
    def get(self, request: Request, guest_id) -> Response:
        ...
        if not is_advisory and guest.artist.user_id != request.user.id:
            return Response(
                {"detail": "You cannot access this Guest accommodation information."},
                status=status.HTTP_403_FORBIDDEN,
            )
        accommodations = Accommodation.objects.filter(guest=guest)
        return Response(AccommodationSerializer(accommodations, many=True).data)

    def post(self, request: Request, guest_id) -> Response:
        if not request.user.roles.filter(role__code=RoleType.ADVISORY).exists():
            return Response(
                {"detail": "Only Advisory users can manage accommodation information."},
                status=status.HTTP_403_FORBIDDEN,
            )
