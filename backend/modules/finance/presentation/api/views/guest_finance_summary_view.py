...
from modules.finance.application.services import GuestFinanceService
...
class GuestFinanceSummaryView(APIView):
    def get(self, request: Request, guest_id) -> Response:
        ...
        if not is_advisory and guest.artist.user_id != request.user.id:
            return Response(
                {"detail": "You cannot access this Guest finance information."},
                status=status.HTTP_403_FORBIDDEN,
            )
        summary = GuestFinanceService.summarize(guest)
        entries = summary.pop("entries")
        return Response({**summary, "entries": FinancialEntrySerializer(entries, many=True).data})
